...
const NOISE_TITLE_PATTERNS: RegExp[] = [
  ...
  // 事業者向け（＝支援を"受ける側"）の導線。診断士が応募する募集ではない。
  /相談会/, // 窓口相談会・無料相談会など（"相談員"は会でないので当たらない）
  /相談したい/, // 「専門家と相談したい」等の事業者向けリンク
  /(アワード|表彰|コンテスト|大賞|グランプリ)/, // 事業者向けの応募・表彰
  /認定(品|製品)|ブランド認定/, // 認定品募集
  /(受入|派遣先)企業.{0,8}募集/, // 事業を受け入れる"企業"の募集＝事業者向け
  /活用(事例|する場合|方法)|導入事例/, // 事業者の活用事例・活用ガイド
  /資金を?調達したい|融資・資金調達/, // 資金調達の導線
  /大募集/, // 「〇〇プロジェクト大募集」等の事業者向けキャンペーン
  // サイトのナビ・システム系リンク（そもそも募集ではない）
  /会員登録|友達募集|登録情報.{0,4}(変更|ご変更)|検索サイト|ログイン/,
];

export function isNoiseTitle(title: string, source: SourceConfig): boolean {
  if (NOISE_TITLE_PATTERNS.some((re) => re.test(title))) return true;
  // 補助金・助成金は事業者向けが大半。ただし「審査員・評価委員・選定委員」は
  // 診断士が担う"支援側"なので残す。
  if (/(補助金|助成金|助成事業)/.test(title) && !/(審査|評価委員|選定委員)/.test(title)) return true;
